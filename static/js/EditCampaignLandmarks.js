...
function createDraggableObject(objectType, objectName, x, y) {
    ...
    obj.id = `${objectType}-${objectName}-${nextObjectId++}`; // each object is the type, type asset nameL, then an arbitrary id value to differentiate
...
function updateGridData() {
    ...
    const grid = [];
    const curGridRows = campaign.regionMapNames.length;
    const curGridCols = campaign.regionMapNames[0].length;
        ...
        for (let col = 0; col < gridData[row].length; col++) {
            ...
            if (cellData.objects && cellData.objects.length > 0) {
                cellData.objects.forEach(obj => {
                    // Find the index of the object type. The data transferred and the data contained within the grid expects an object name.
